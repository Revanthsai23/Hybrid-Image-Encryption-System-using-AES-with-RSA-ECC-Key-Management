...
class AES_encryption:
    def __init__(self, image, master_key) -> None:
        ...
        if len(self.image_data.shape) == 3:
            self.is_colour = True
            self.image_metadata_info = image_metadata(
                self.image_data.shape[0], self.image_data.shape[1], self.is_colour, self.image_data.shape[2])

        else:
            self.is_colour = False
            self.image_metadata_info = image_metadata(
                self.image_data.shape[0], self.image_data.shape[1], self.is_colour, 0)
    ...
    def encrypt_image(self, image_array):
        ...
        cipher_image = cipher_image.reshape(
            reshape_for_image(len(cipher_image)))
    ...
    def aes_encryption(self):
        if not self.is_colour:
            ...
            image = cv2.merge(
                (blue_channel_encrypted, green_channel_encrypted, red_channel_encrypted))
