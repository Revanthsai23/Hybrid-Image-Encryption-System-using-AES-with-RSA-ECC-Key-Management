...
def hmac(key: bytearray, msg: bytearray, block_s: int = 128,
    ...
    for i in range(block_s):
        ipadkey.append(new_key[i] ^ 0x36)  # 0x36 = 54
        opadkey.append(new_key[i] ^ 0x5c)  # 0x5c = 92
...
class Ecdsa:
    ...
    def recover_pubkey(self, m_hash, signature):
        ...
        for i in range(2):
            ...
            yreg = list(point_add(y_r[0], y_r[1],
                        e_g[0], e_g[1], self.q, self.a))
    ...
    def verify_signature(self, signature: tuple, m_hash: int, a_pub_key: tuple):
        ...
        if self.unauth_sign == (0, 1):
            raise ValueError(
                "generated verification signature is point at infinity")
